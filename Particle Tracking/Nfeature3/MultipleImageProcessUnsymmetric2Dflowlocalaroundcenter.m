...
for i=1:len
    x1=-binvelocity(i,1);
    y1=binvelocity(i,2);
    vx=-binvelocity(i,3)*60;
    vy=binvelocity(i,4)*60;
    gamma=(vx-vy)/(x1+y1);
    alpha=(vx+vy)/(gamma*(x1-y1));
    if and(abs(gamma)<=5,abs(alpha)<=5)&& and(abs(binvelocity(i,1))<=50,abs(binvelocity(i,2))<=50)
       result(m,1)=x1;
       result(m,2)=y1;
       result(m,3)=gamma;
       result(m,4)=alpha;
...
for i=1:len(1)
    ...
    bigerror(m,1)=result(i,1);
    bigerror(m,2)=result(i,2);
    bigerror(m,3)=abs((result(i,3)-mean(result(:,3)))/mean(result(:,3)))*100;
    m=m+1;
end

% length=size(bigerror);
% for i = 1:length(1)
% A(bigerror(i,1),bigerror(i,2)) = bigerror(i,3);               %populate the Matrix
% end
% Z = magic(5);
% h = bar3(A);
% colormap cool
% bigerror(:,1)=(bigerror(:,1)+305)/10;
% bigerror(:,2)=(bigerror(:,2)+269)/10; 