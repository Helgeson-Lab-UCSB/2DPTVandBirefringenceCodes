function s = bpass2(img,lambda,w)
...
% 		w: A length in pixels somewhat larger than a typical object. Must be an odd valued 
...
factor=(sum(xpt.^2)-1/N);  %sum(xpt.^2)=1/Bexp(-(i^2+j^2)/(4*lambda^2))
%factor=(sum(xpt.^2)-1/(N^2));
gx = xpt;    
gy=gx';
bx = zeros(1,N)-1./N;  
% bx = zeros(1,N)-1./(N^2);
