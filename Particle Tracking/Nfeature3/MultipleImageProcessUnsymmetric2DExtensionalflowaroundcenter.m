...
% xcenter=300;
% ycenter=264;
% velocity(:,1)=velocity(:,1)-xcenter;
% velocity(:,2)=velocity(:,2)-ycenter;
% range=100;
a=size(velocity);
% m=1;
% centervelocity=[];
% for i=1:a(1)
%    if and(velocity(i,1)>=(-range/2),velocity(i,1)<=range/2) && and(velocity(i,2)>=(-range/2),velocity(i,2)<=range/2)
%        centervelocity(m,1)=velocity(i,1);
%        centervelocity(m,2)=velocity(i,2);
%        centervelocity(m,3)=velocity(i,3);
%        centervelocity(m,4)=velocity(i,4);
%        m=m+1;
%    end
% end
...
binvelocity(:,1)=binvelocity(:,1)*10-300;
binvelocity(:,2)=binvelocity(:,2)*10-264;
len=size(binvelocity);
shearrate=[];
alpha=1;
truealpha=0.39;
m=1;
for i=1:len
    x1=binvelocity(i,1);
    y1=binvelocity(i,2);
    x2=binvelocity(i,1)+binvelocity(i,3);
    y2=binvelocity(i,2)+binvelocity(i,4);
    ...
    if and(abs(calc1)<=10,abs(calc2)<=10)&& and(abs(binvelocity(i,1))<=50,abs(binvelocity(i,2))<=50)
%     if and(abs(calc1)<=10,abs(calc2)<=10)&& abs(binvelocity(i,1))<=5
        shearrate(m,1)=x1;
        shearrate(m,2)=y1;
        shearrate(m,3)=calc1;
        shearrate(m,4)=calc2;
...
% r=size(centervelocity);
%  figure
%   for t=1:r(1)
%      plot(centervelocity(t,1),centervelocity(t,3)*60,'k')
%      plot(centervelocity(t,2),centervelocity(t,4)*60,'r')
%      hold on;
%   end
...
len=size(shearrate);
for i=1:len(1)
    error=abs(shearrate(i,3)-mean(shearrate(:,3)));
    if error>=std(shearrate(:,3))
        bigerror(m,1)=shearrate(i,1);
        bigerror(m,2)=shearrate(i,2);
        m=m+1;
    end
end