...
for i=1:goodparticle
    ...
    kkkk=size(temp);
    if (kkkk(1)>0) && and(mean(temp(:,1))>=0,mean(temp(:,1))<=0.35)
...
for i=1:u
    ...
    for j=1:r(1)-1
      x1=p1(j,1);
      y1=p1(j,2);
      x2=x1+(p1(j+1,1)-p1(j,1))/(p1(j+1,3)-p1(j,3));
      y2=y1+(p1(j+1,2)-p1(j,2))/(p1(j+1,3)-p1(j,3));
      calc1=framerate/(p1(j+1,3)-p1(j,3))/sqrt(alpha)*asinh(2*sqrt(alpha)*(x2*y1-x1*y2)/((x1^2+y1^2)*(1-alpha)+2*(1+alpha)*x1*y1));
      g(m,1)=p1(j,3);
      g(m,2)=calc1;
      m=m+1;
    end
