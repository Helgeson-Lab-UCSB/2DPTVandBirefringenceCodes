...
for i=1:len
    ...
    if and(abs(gamma)<=0.06,abs(alpha)<=2)  && and(gamma>0,alpha>0)
       result(m,1)=x1;
       result(m,2)=y1;
       result(m,3)=gamma;
       result(m,4)=alpha;
...
% o=size(result);
% for i=1:o(1)
%     if and(result(i,3)>=0,result(i,3)<=0.1)
%         group(m,1)=result(i,1);
%         group(m,2)=result(i,2);
%         group(m,3)=result(i,3);
%         group(m,4)=result(i,4);
%         m=m+1;
%     end
% end
...
% y=[];
% c=[];
% x=result(:,1)+ximagecenter;
% y=result(:,2)+(480-yimagecenter);
% c=result(:,3);
% 
% colormap jet;
