%allocate memory
matbinvelu=zeros(imagexres/(yres*velbinsize)+1,imageyres/(yres*velbinsize)+1);
matbinvelv=zeros(imagexres/(yres*velbinsize)+1,imageyres/(yres*velbinsize)+1);
binvelocity=zeros(imagexres*imageyres/(velbinsize*velbinsize),3);
...
errorux=zeros(imagexres*imageyres/(yres*yres),3);
...
continuity=zeros(imagexres/(xres*gradbinsize)+1,imageyres/(xres*gradbinsize)+1);
...
disp(strcat('Calculating FTP and MVGT...'));
...
figure
colormap jet;
scatter((G(:,1)*gradbinsize-xcenter)*lengthperpix,(G(:,2)*gradbinsize-ycenter)*lengthperpix,gradbinsize,G(:,3),'filled','s')
caxis([0,200])
colorbar;axis equal;axis([(0-xcenter)*lengthperpix (imagexres-xcenter)*lengthperpix (0-ycenter)*lengthperpix (imageyres-ycenter)*lengthperpix]);
xlabel(lengthunit)
ylabel(lengthunit)
title(colorbar,strcat('G'))
