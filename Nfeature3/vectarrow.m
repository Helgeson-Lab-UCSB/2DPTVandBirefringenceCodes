...
%   $Revision: 1.0
  ...
  if max(size(p0))==3
      if max(size(p1))==3
          ...
          y1 = p1(2);
          z1 = p1(3);
          plot3([x0;x1],[y0;y1],[z0;z1],'r','LineWidth',1.5);   % Draw a line between p0 and p1
  ...
  elseif max(size(p0))==2
      if max(size(p1))==2
          ...
          x1 = p1(1);
          y1 = p1(2);
          plot([x0;x1],[y0;y1],'r','LineWidth',1.5);   % Draw a line between p0 and p1
          ...
          hold on
          plot(hu(:),hv(:),'r','LineWidth',1.5)  % Plot arrow head
          grid on
          xlabel('x')
          ylabel('y')
          hold off
