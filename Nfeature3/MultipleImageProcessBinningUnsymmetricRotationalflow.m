...
for i=1:max(lub(:,4))
    ...
    n=1;
    for j=100:o(1)
        xerror=(pt(j,1)-x0)^2/(x0)^2;
        yerror=(pt(j,2)-y0)^2/(y0)^2;
        if and(xerror<=0.001,yerror<=0.001)
            u(n,1)=pt(j,4);
            u(n,2)=pt(j,3)/6;
            n=n+1;
        end
...
% binvelocity(:,2)=binvelocity(:,2)-26;
% len=size(binvelocity);
% shearrate=[];
% 
% m=1;
% for i=1:len
%     x=binvelocity(i,1)*10;
%     y=binvelocity(i,2)*10;
%     vx=60*binvelocity(i,3);
%     vy=60*binvelocity(i,4);
%     r=sqrt(x^2+y^2);
%     v=sqrt(vx^2+vy^2);
%     gamma=v/r;
%     shearrate(m,1)=x;
...
%  figure
%   for t=1:r(1)
%      plot(shearrate(t,3),shearrate(t,4),'k')
%      hold on;
%   end
%   
%   
