...
binvelocity(:,1)=binvelocity(:,1)-30;
binvelocity(:,2)=binvelocity(:,2)-28;
len=size(velocity);
shearrate=[];
alpha=0.84;
% truealpha=0.885;
...
for i=1:len
    x1=velocity(i,1);
    y1=velocity(i,2);
    x2=velocity(i,1)+velocity(i,3);
    y2=velocity(i,2)+velocity(i,4);
    calc1=framerate/trajectorygap/sqrt(alpha)*asinh(2*sqrt(alpha)*(x2*y1-x1*y2)/((x1^2+y1^2)*(1-alpha)+2*(1+alpha)*x1*y1));
%     calc2=framerate/trajectorygap/sqrt(truealpha)*asinh(2*sqrt(truealpha)*(x2*y1-x1*y2)/((x1^2+y1^2)*(1-truealpha)+2*(1+truealpha)*x1*y1));
    if and(abs(calc1)<=0.5,calc1>0)
        shearrate(m,1)=x1;
        shearrate(m,2)=y1;
        shearrate(m,3)=calc1;
%         shearrate(m,4)=calc2;
...
len=size(shearrate);
for i=1:len(1)
    error=abs(shearrate(i,4)-mean(shearrate(:,4)));
    if error>=std(shearrate(:,4))
